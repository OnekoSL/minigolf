...
const OUTPUT := "res://.godot/arrow-harmony"
...
func _capture() -> void:
	...
	for index in range(9):
		var definition := catalog.get_hole(StringName("arrow_armageddon_%02d" % (index + 1)))
		...
		print("Pfeil-Detail %d: %s" % [index + 1, error_string(result)])
		detail.queue_free()
	await process_frame
	await RenderingServer.frame_post_draw
	var result := sheet.get_texture().get_image().save_png(OUTPUT + "/uebersicht.png")
	print("Pfeil-Gesamtansicht: ", error_string(result))
	quit()
