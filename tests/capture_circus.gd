...
func _capture() -> void:
	...
	for course in [CourseCatalog.load_default().get_course(&"zirkus_course")]:
	...
	for page in range(3):
		...
		if page == 2: app._select_option(2)
		await get_tree().process_frame
		await RenderingServer.frame_post_draw
	...
	for course in [CourseCatalog.load_default().get_course(&"zirkus_course")]:
		for index in range(9):
			...
			if index == 4:
				var elephant := app.gameplay.hole.obstacle_nodes[0] as ElephantObstacle
				elephant.set_physics_process(false)
				app.gameplay.ball.set_physics_process(false)
				for phase in [0.5,2.0,4.5]:
					elephant.reset_motion()
					for tick in range(roundi(phase * 60)):
						elephant.advance_motion(1.0 / 60.0)
					await get_tree().process_frame
					await RenderingServer.frame_post_draw
					get_tree().root.get_texture().get_image().save_png("%s/elefant-phase-%.1f.png" % [OUTPUT,phase])
				elephant.reset_motion()
				...
				for tick in range(138):
					elephant.advance_motion(1.0 / 60.0)
					ball._physics_process(1.0 / 60.0)
					if tick in [3,65,75]:
						await get_tree().process_frame
						await RenderingServer.frame_post_draw
						get_tree().root.get_texture().get_image().save_png("%s/elefant-transfer-%d.png" % [OUTPUT,tick])
				app.gameplay.restart_hole()
				elephant.set_physics_process(true)
