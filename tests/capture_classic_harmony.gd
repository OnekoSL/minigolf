...
const OUTPUT := "res://.godot/classic-harmony"
...
func _capture() -> void:
	...
	for index in range(9):
		var definition := catalog.get_hole(StringName("classic_nine_%02d" % (index + 1)))
		...
		print("Klassik-Detail %d: %s" % [index + 1, error_string(result)])
		detail.queue_free()
	await process_frame
	await RenderingServer.frame_post_draw
	var result := sheet.get_texture().get_image().save_png(OUTPUT + "/uebersicht.png")
	print("Klassik-Gesamtansicht: ", error_string(result))
	quit()
